...
\item
  ...
  \item
	  ...
	  \begin{itemize}
		  ...
		  \item else:
			  \begin{itemize}
				  \item perform interaction with ghost. Update the local and the ghost money variable.
%				  \item if countepart id is not None: perform interaction with ghost. Record new money of ghost in RL2
...
At each interaction, the acting agent updates its partner money holding either if it is a local agent or a ghost.
...
This allows ghosts, like local agents, be involved in several interactions. In this version of the model all the agents of a rank has at least one interaction.


\subsection{All ranks at a time}
A full parallel implementation of model 2 with Repast4py ghosts can be as follows:
...
\item
  ...
  \item 
	  all ranks analyze the ghosted list and identify ghosts to be removed. Each agent has to be ghosted at most in one rank
  \item 
	  all ranks remove ghost signaled by other ranks
  \item
	  each agent: perform interaction
	  \begin{itemize}
		  \item if counterpartRank is equal to current rank: perform local interaction and update money holdings of the two local agents
...
Now we have to make sure that each agent is ghosted to at most in one rank. The ghosted list can help in identifying if an agent is
ghosted in more than a rank. We have to delete ghost in such a way that
the agent has only one ghost.

Now exchanges can be performed. Money holdings are accounted
